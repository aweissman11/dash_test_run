...
function Hit({ hit }: { hit: (IPost | IPage) & BaseHit }) {
  ...
  const defaultLanguage = fallbackLanguage as CONTENTFUL_DEFAULT_LOCALE_CODE;
  ...
  if (type === "page") {
    ...
    linkTo = `/contentful/${hit.fields.slug["en-US"]}`;
    const imageField = (hit as IPost).fields.image;
    if (imageField) {
      imageProps = getImageProps(imageField[defaultLanguage], { locale });
    }
  } else {
